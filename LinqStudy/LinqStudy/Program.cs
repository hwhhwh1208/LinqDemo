    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Student> studentList = new List<Student>() { new Student() { StudentID = "1", StudentName = "张三", Age = 18 },
                                                              ...
                                                              new Student() { StudentID = "4",StudentName = "张三",Age = 21},
                                                              new Student() { StudentID = "5",StudentName = "李四",Age = 22},
                                                              new Student() { StudentID = "6",StudentName = "王五",Age = 23},};
        ...
        public static void TestDistinct(List<Student> stuList)
        {
            var stuData = from stus in stuList.Distinct(new StudentCompare())
                          select stus;

            foreach (var item in stuData)
            {
                Console.WriteLine("学生ID：" + item.StudentID + "---学生姓名：" + item.StudentName);
            }
        }
